...
void string_to_spv_func(const std::string& _name, const std::string& in_fname, const std::map<std::string, std::string>& defines, bool fp16 = true, bool coopmat = false, bool coopmat2 = false, bool f16acc = false, const std::string& suffix = "") {
    std::string name = _name + (f16acc ? "_f16acc" : "") + (coopmat ? "_cm1" : "") + (coopmat2 ? "_cm2" : (fp16 ? "" : "_fp32")) + suffix;
...
void string_to_spv(const std::string& _name, const std::string& in_fname, const std::map<std::string, std::string>& defines, bool fp16 = true, bool coopmat = false, bool coopmat2 = false, bool f16acc = false, const std::string& suffix = "") {
    {
        // _name = _name + (f16acc ? "_f16acc" : "") + (coopmat ? "_cm1" : "") + (coopmat2 ? "_cm2" : (fp16 ? "" : "_fp32")) + suffix;
    ...
    compiles.push_back(std::async(string_to_spv_func, _name, in_fname, defines, fp16, coopmat, coopmat2, f16acc, suffix));
}
...
void write_output_files() {
    ...
    for (const char *op : {"add", "sub", "mul", "div", "add_rms"}) {
        ...
        std::string data = "unsigned char *" + std::string(op) + "_data[2][2][2] = ";
        std::string len = "uint64_t " + std::string(op) + "_len[2][2][2] = ";
    ...
    for (const auto& tname : type_names) {
        ...
        fprintf(hdr, "extern unsigned char *arr_dmmv_id_%s_%s_f32_data[3];\n", tname.c_str(), btype.c_str());
        fprintf(hdr, "extern uint64_t arr_dmmv_id_%s_%s_f32_len[3];\n", tname.c_str(), btype.c_str());
        std::string data_id = "unsigned char *arr_dmmv_id_" + tname + "_" + btype + "_f32_data[3] = {mul_mat_vec_id_" + tname + "_" + btype + "_f32_data, mul_mat_vec_id_" + tname + "_" + btype + "_f32_subgroup_data, mul_mat_vec_id_" + tname + "_" + btype + "_f32_subgroup_no_shmem_data};\n";
        std::string len_id =  "uint64_t arr_dmmv_id_"       + tname + "_" + btype + "_f32_len[3] =  {mul_mat_vec_id_" + tname + "_" + btype + "_f32_len,  mul_mat_vec_id_" + tname + "_" + btype + "_f32_subgroup_len, mul_mat_vec_id_" + tname + "_" + btype + "_f32_subgroup_no_shmem_len};\n";
        fputs(data_id.c_str(), src);
        fputs(len_id.c_str(), src);
    }
    }
...
int main(int argc, char** argv) {
    ...
    if (args.find("--glslc") != args.end()) {
        ...
        // int has_glslc = 2;
        // if (!std::filesystem::exists(GLSLC)) {
            // std::cerr << "Error: glslc not found at " << GLSLC << std::endl;
            // --has_glslc;
        // }
        // if (!std::filesystem::is_regular_file(GLSLC)) {
            // std::cerr << "Error: glslc is not a regular file " << GLSLC << std::endl;
            // --has_glslc;
        // }
