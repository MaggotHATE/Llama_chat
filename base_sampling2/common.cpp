...
struct common_init_result common_init_from_params(common_params & params) {
    ...
    if (params.reranking) {
        ...
        if (llama_token_bos(model) == LLAMA_TOKEN_NULL) {
            printf("%s: warning: model does not have a  BOS token, reranking will not work\n", __func__);
            ok = false;
        }

        if (llama_token_eos(model) == LLAMA_TOKEN_NULL) {
            printf("%s: warning: model does not have an EOS token, reranking will not work\n", __func__);
            ok = false;
        }

        if (llama_token_sep(model) == LLAMA_TOKEN_NULL) {
            printf("%s: warning: model does not have a  SEP token, reranking will not work\n", __func__);
            ok = false;
        }
    ...
    if (params.sparams.ignore_eos && llama_token_eos(model) == LLAMA_TOKEN_NULL) {
        printf("%s: warning: model does not have an EOS token, ignoring --ignore-eos\n", __func__);
