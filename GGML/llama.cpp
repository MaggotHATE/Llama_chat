...
std::string llama_token_to_string_with_model(const struct llama_model * model, llama_token token) {
    if (token >= llama_n_vocab_from_model(model)) {
        return nullptr;
    }
    
    return model->vocab.id_to_token[token].tok;
}
    
std::string llama_token_to_string(const struct llama_context * ctx, llama_token token) {
    return llama_token_to_string_with_model(&ctx->model, token);
}
