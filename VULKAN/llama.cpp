...
#   define llama_host_malloc(n)  ggml_vk_host_malloc(n)
#   define llama_host_free(data) ggml_vk_host_free(data)
#else
...
struct llama_context * llama_new_context_with_model(
    ...
    LLAMA_LOG_INFO("%s: memory_type init\n", __func__);
    ...
    if (!hparams.vocab_only) {
        if (!llama_kv_cache_init(ctx->model.hparams, ctx->kv_self, memory_type, cparams.n_ctx, model->n_gpu_layers)) {
            ...
            llama_free(ctx);
            return nullptr;
        } else LLAMA_LOG_INFO("%s: llama_kv_cache_init() starting... \n", __func__);
